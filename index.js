			...
			try {
				...
				if (false!==config.debug) {
					self.express.get(endpoint,function(req,res) {
						...
						else if (typeof req.param('state')!="undefined") {
						  var redirect = req.param('redirect_uri');
              var client_id = req.param('client_id');
              var state = req.param('state');
							res.set('Content-Type', 'text/html').send(
							  "<html><body><p>This doesn't make any sense, but you have to log in on a non-mobile device - go to alexa.amazon.com on a web browser and access this card there if you're in the mobile app. <a href='https://platform.adstage.io/oauth/authorize?client_id=" +
							    client_id + "&state=" + 
							    state + "&response_type=code&redirect_uri=" + 
							    encodeURIComponent(redirect) + "'>Redirect to login</a>"
							);
						}
						else {
